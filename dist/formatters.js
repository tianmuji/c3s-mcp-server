...
exports.formatCaseList = formatCaseList;
exports.formatCaseDetail = formatCaseDetail;
exports.formatCaseLifecycle = formatCaseLifecycle;
exports.formatCaseHistory = formatCaseHistory;
exports.formatCustomerInfo = formatCustomerInfo;
...
function formatCaseList(data) {
    ...
    for (const item of list) {
        ...
        if (item.product_version)
            lines.push(`  版本: ${item.product_version}`);
        if (item.hardware_info)
            lines.push(`  设备: ${item.hardware_info}`);
        if (item.customer_info?.email)
            lines.push(`  客户邮箱: ${item.customer_info.email}`);
        if (item.user_info?.name)
            lines.push(`  处理人: ${item.user_info.name}`);
...
function formatCaseDetail(data) {
    ...
    if (ci.product_version)
        lines.push(`版本: ${ci.product_version}`);
    if (ci.hardware_info)
        lines.push(`设备: ${ci.hardware_info}`);
    if (ci.language)
        lines.push(`语言: ${ci.language}`);
    if (ci.locale)
        lines.push(`地区: ${ci.locale}`);
    if (ci.ip)
        lines.push(`IP: ${ci.ip}`);
    ...
    if (ci.customer_info) {
        ...
        lines.push("\n--- 客户信息 ---");
        if (c.email)
            lines.push(`邮箱: ${c.email}`);
        if (c.phone)
            lines.push(`电话: ${c.phone}`);
        if (c.name)
            lines.push(`姓名: ${c.name}`);
        if (c.company)
            lines.push(`公司: ${c.company}`);
    ...
    if (ci.email && ci.email.length > 0) {
        ...
        for (let i = 0; i < ci.email.length; i++) {
            ...
            if (em.to_address)
                lines.push(`  收件人: ${em.to_address}`);
            if (em.cc_address)
                lines.push(`  抄送: ${em.cc_address}`);
...
function formatCaseLifecycle(data) {
...
/** Format case history */
function formatCaseHistory(data) {
    const list = data?.list;
    const total = data?.total;
    if (!Array.isArray(list) || list.length === 0) {
        return "该客户暂无历史工单。";
    }
    const lines = [`客户历史工单 (共 ${total || list.length} 条)\n`];
...
function formatCustomerInfo(data) {
    if (!data)
        return "客户信息不存在。";
    const lines = ["客户信息:\n"];
    ...
    if (data.company)
        lines.push(`公司: ${data.company}`);
    if (data.level)
        lines.push(`等级: ${data.level}`);
    if (data.remark)
        lines.push(`备注: ${data.remark}`);
    if (data.product_id && Array.isArray(data.product_id)) {
        lines.push(`产品: ${data.product_id.map((id) => getProductName(id)).join(", ")}`);
    }
    if (data.platform_id && Array.isArray(data.platform_id)) {
        lines.push(`平台: ${data.platform_id.map((id) => getPlatformName(id)).join(", ")}`);
    }
